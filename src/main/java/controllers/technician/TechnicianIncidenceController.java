...
public class TechnicianIncidenceController extends AbstractController {
	...
	public ModelAndView list(final Integer pageSize) {
		...
		result.addObject("requestUri", "incidence/technician/list.do");
			...
			public ModelAndView delete(@Valid final IncidenceForm incidence, final BindingResult binding) {
				...
				try {
					...
					if (ooops.getMessage().equals("msg.not.loged.block")) 
						result = this.createEditModelAndView(incidence, "msg.not.loged.block", incidence.getUser().getCustomer().getId());
					else if (ooops.getMessage().equals("msg.not.owned.block")) 
						result = this.createEditModelAndView(incidence, "msg.not.owned.block", incidence.getUser().getCustomer().getId());
					else
						result = this.createEditModelAndView(incidence, "msg.commit.error", incidence.getUser().getCustomer().getId());
				}
				return result;
			}
	...
	protected ModelAndView createEditModelAndView(final IncidenceForm incidence, final String message, int customerId) {
		...
		result.addObject("requestUri", "incidence/technician/edit.do");
